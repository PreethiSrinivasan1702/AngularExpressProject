...
function displayAllUsers(request,response)
{
    ...
    mysqlconnection.query('Select * from User',processResults);
...
function getUserById(request, response){
    ...
    //Parameterqized SQL
    mysqlconnection.query('select * from User where userid = ?', [userid], processResults);
    response.send(queryresults);
}
...
function getUserByEmail(request, response){
    ...
    //Parameterqized SQL
    mysqlconnection.query('select * from User where emailid = ?', [email], processResults);
    response.send(queryresults);
}
...
function insertUser(request, response){
    ...
    mysqlconnection.query('Insert into User values (?, ?, ?)', [userid, password, emailid], checkInsertStatus);
...
function updateUser(request, response){
    ...
    var password = request.body.password;
    var emailid = request.body.emailid;
    mysqlconnection.query('Update User SET userid = ?, password = ?, emailid = ? where userid = ?', [userid, password, emailid, userid], checkUpdateStatus);
    response.send(JSON.stringify(statusMessage));
}
...
function deleteUser(request, response){
    var userid = request.body.userid;
    mysqlconnection.query('Delete from User where userid = ?', [userid], checkDeleteStatus);
    response.send(JSON.stringify(statusMessage));
}
app.post('/deleteUser', bparserInit, deleteUser);
